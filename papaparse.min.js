...
!function(e, t) {
		...
		if (t.dynamicTyping = i,
			...
			var e =
				(i = f.URL || f.webkitURL || null,
				 r = s.toString(),
				 b.BLOB_URL ||
				     (b.BLOB_URL = i.createObjectURL(new Blob(
					  ['(', r, ')();'],
		    ...
		    return r.userStep = t.step, r.userChunk = t.chunk,
			   r.userComplete = t.complete, r.userError = t.error,
			   t.step = M(t.step), t.chunk = M(t.chunk),
			   t.complete = M(t.complete), t.error = M(t.error),
			   delete t.worker,
			   void r.postMessage(
			       {input: e, config: t, workerId: r.id})
		}
		var n = null;
		b.NODE_STREAM_INPUT,
		    'string' == typeof e ?
	...
	b.UNIT_SEP = String.fromCharCode(31), b.BYTE_ORDER_MARK = '\ufeff',
	b.BAD_DELIMITERS = ['\r', '\n', '"', b.BYTE_ORDER_MARK],
	b.WORKERS_SUPPORTED = !n && !!f.Worker, b.NODE_STREAM_INPUT = 1,
	b.LocalChunkSize = 10485760, b.RemoteChunkSize = 5242880,
	b.DefaultDelimiter = ',', b.Parser = E, b.ParserHandle = i,
	b.NetworkStreamer = l, b.FileStreamer = c, b.StringStreamer = p,
	b.ReadableStreamStreamer = g, f.jQuery) {
	var d = f.jQuery;
	d.fn.parse = function(o) {
		...
		if (0 !== h.length) {
			...
			if ('object' == typeof s) {
			    if ('abort' === s.action)
				return e = 'AbortError', t = n.file,
				       i = n.inputElem, r = s.reason,
				       void (
					   M(o.error) &&
					   o.error({name: e}, t, i, r));
			    if ('skip' === s.action)
				return void u();
			    'object' == typeof s.config &&
				(n.instanceConfig =
				     d.extend(n.instanceConfig, s.config))
			} else if ('skip' === s)
			    return void u()
		    }
	...
	this._completeResults = {data: [], errors: [], meta: {}}, function(e) {
		...
		if (o)
		    f.postMessage(
			{results: n, workerId: b.WORKER_ID, finished: a});
		...
		return this._config.step || this._config.chunk ||
		       ...
		       this._completed || !a || !M(this._config.complete) ||
			   n && n.meta.aborted ||
			   (this._config.complete(
				this._completeResults, this._input),
			    this._completed = !0),
		       a || n && n.meta.paused || this._nextChunk(), n
	    }
	    this._halted = !0
	}, this._sendError = function(e) {
	    M(this._config.error) ? this._config.error(e) :
				    o && this._config.error &&
		    f.postMessage(
			{workerId: b.WORKER_ID, error: e, finished: !1})
	}
    }
    function l(e) {
	var r;
	...
	this.stream = function(e) {
	    ...
	    s ? ((r = new FileReader).onload = v(this._chunkLoaded, this),
		 r.onerror = v(this._chunkError, this)) :
	    ...
	    if (this._config.chunkSize) {
		var t = Math.min(
		    this._start + this._config.chunkSize, this._input.size);
	...
	u.call(this, e = e || {}), this.stream = function(e) {
		...
		return t ? (e = i.substring(0, t), i = i.substring(t)) :
			   (e = i, i = ''),
		       this._finished = !i, this.parseChunk(e)
	    }
	}
    }
    function g(e) {
	...
	function g() {
	    if (c && h &&
		    (k('Delimiter', 'UndetectableDelimiter',
		...
		function e(e, t) {
		    ...
		    return m.header &&
			       (i > l.length ?
				    k('FieldMismatch', 'TooManyFields',
				      'Too many fields: expected ' + l.length +
					  ' fields but parsed ' + i,
				      f + t) :
				    i < l.length &&
					k('FieldMismatch', 'TooFewFields',
					  'Too few fields: expected ' +
					      l.length + ' fields but parsed ' +
					      i,
					  f + t)),
			   r
		}
	...
	function v(e, t) {
	    return i = e,
		   m.dynamicTypingFunction && void 0 === m.dynamicTyping[i] &&
	...
	this.parse = function(e, t, i) {
		...
		var n =
		    ...
		    return {
			successful: !!(m.delimiter = s), bestDelimiter: s
		    }
		}(e, m.newline, m.skipEmptyLines, m.comments,
		    m.delimitersToGuess);
		n.successful ? m.delimiter = n.bestDelimiter :
			       (h = !0, m.delimiter = b.DefaultDelimiter),
			       c.meta.delimiter = m.delimiter
	    }
	    var s = w(m);
	    return m.preview && m.header && s.preview++,
		   a = e, o = new E(s), c = o.parse(a, t, i), g(),
		   d ? {meta: {paused: !0}} : c || {
		meta: {paused: !1}
	    }
	    ...
	    e = !0, o.abort(), c.meta.aborted = !0,
	    M(m.complete) && m.complete(c), a = ''
	}
    }
    function j(e) {
	return e.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')
    ...
    function E(e) {
	var S, O = (e = e || {}).delimiter, x = e.newline, I = e.comments,
	       T = e.step, D = e.preview, A = e.fastMode,
	...
	this.parse = function(r, t, i) {
	    ...
	    var n = r.length, e = O.length, s = x.length, a = I.length,
		o = M(T), h = [], u = [], f = [], d = F = 0;
	    if (!r)
		return C();
		...
		for (var l = r.split(x), c = 0; c < l.length; c++) {
			...
			if (o) {
			    if (h = [], k(f.split(O)), R(), z)
				return C()
			} else
			    k(f.split(O));
			if (D && D <= c)
			    return h = h.slice(0, D), C(!0)
		    }
		}
		return C()
	    }
	    for (var p = r.indexOf(O, F), g = r.indexOf(x, F),
		     _ = new RegExp(j(L) + j(S), 'g'), m = r.indexOf(S, F);
		 ;)
		if (r[F] !== S)
			...
			if (-1 === (m = r.indexOf(S, m + 1)))
				...
				index: F
				   ...
				   E();
			if (m === n - 1)
			    return E(r.substring(F, m).replace(_, S));
			if (S !== L || r[m + 1] !== L) {
			    if (S === L || 0 === m || r[m - 1] !== L) {
				-1 !== p && p < m + 1 &&
				    (p = r.indexOf(O, m + 1)),
				...
				var v = b(g);
				if (r.substring(m + 1 + v, m + 1 + v + s) ===
				    x) {
				    if (f.push(r.substring(F, m).replace(_, S)),
					w(m + 1 + v + s), p = r.indexOf(O, F),
					m = r.indexOf(S, F), o && (R(), z))
					return C();
				    if (D && h.length >= D)
					return C(!0);
				    break
	    ...
	    function E(e) {
		return i ||
			   (void 0 === e && (e = r.substring(F)), f.push(e),
			    F = n, k(f), o && R()),
		       C()
	    }
	    function w(e) {
    ...
    return o &&
	       (f.onmessage =
		    function(e) {
			var t = e.data;
			void 0 === b.WORKER_ID && t &&
			    (b.WORKER_ID = t.workerId);
